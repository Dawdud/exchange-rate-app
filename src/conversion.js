...
export class Conversion
{
    ...
    convert(currencyValue, desiredCurrencyValue)
 {
                ...
                if(this.digitRegex.test(handler.input.value)) {
                    ...
                    let ConvertedCurrency= Math.round((currencyValue*inputint/desiredCurrencyValue)*100)/100;
    ...
    count()
{
    ...
    handler.submitButton.addEventListener('click', (ev)=>{
            ...
            for(let i=0; i<handler.leftCurrencyField.options.length;i++)
            {
                ...
                if(firstSelectedOption.selected )
                {
                 ...
                 let currencyValue= new CurrencyData(this.data, firstSelectedOption.value).getUrl();
                     this.CurrencyObjects.CurrencyName= currencyValue['name'];


                    if(typeof currencyValue['mid']==='number' && typeof currencyValue['mid']!='undefined')
                    {
                ...
                {
                    let  desiredCurrencyValue= new CurrencyData(this.data, secondSelectedOption.value).getUrl();
                    this.CurrencyObjects.desiredCurrencyName= desiredCurrencyValue['name'];
                    
                     
                    
                    if(typeof desiredCurrencyValue['mid']==='number' && typeof desiredCurrencyValue['mid']!='undefined')
                    {
